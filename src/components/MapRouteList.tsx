							...
							{missedPoints.length > 0 ? (
								<View style={{ flex: 1 }}>
									<Text
										...
										Точки які відсутні на карті
									</Text>
									{missedPoints.map(point => {
										return (
											<PointItem key={point.id}>
												<Text style={{ flex: 1 }}>{point.name}</Text>
												<TouchableOpacity
													onPress={() => addMarkerHandler(point.id)}
													style={{
														padding: 5,
							...
							<View style={{ flex: 1 }}>
								...
								{displayedPoints.map(point => {
									return (
										<PointItem key={point.id}>
											<Text
												style={{ flex: 1, fontWeight: '700', color: '#000' }}>
												{point.name}
											</Text>
											<TouchableOpacity
												onPress={() => moveToMarkerHandler(point.id)}
												style={{
													padding: 7,
													backgroundColor: '#0F0F0F',
													borderRadius: 12,
