...
const SearchByStation = ({ allStations, setResultsData }: Props) => {
	...
	const [
		getRoutes,
		{ data: routesResult, error: routesError, isLoading: routesIsLoading },
	] = useLazyGetRouteByPointsQuery();
	const [selectedFromStation, setSelectedFromStation] =
		useState<IBusStations | null>(null);
	...
	useEffect(() => {
		if (selectedFromStation?.id && selectedToStation?.id) {
			const requestObj = {
				fromPoint: selectedFromStation?.id,
				toPoint: selectedToStation?.id,
			};
	...
	const updateSelectedFromStation = (value: string) => {
		const findedStation = allStations.find(
	...
	const updateSelectedToStation = (value: string) => {
		if (attachedStations) {
			const findedStation = attachedStations.find(
				station => station.stationName === value,
			);
			if (findedStation) {
			...
			{attachedStations && selectedFromStation && (
				<>
					...
					<SearchFieldContainer>
						{selectedToStation ? (
							<SelectedStation
								cancel={() => setSelectedToStation(null)}
								name={selectedToStation.stationName}
							/>
						) : (
							<SearchFieldWithDropdown
								title="Куди"
								itemsForSearch={attachedStations.map(
