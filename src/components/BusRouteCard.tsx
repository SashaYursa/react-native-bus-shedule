...
import { formattingDate, formattingTime } from '../utils/helpers'
...
const BusRouteCard = ({ station, sheduleItem}: Props) => {
    const route = sheduleItem.busRoute.split(' - ').map(str => str.trim().replace('#', '').replace('+', ''))
    ...
    <Container>
        ...
        <RouteInfoContainer>
            <RouteInfoItem>
                <RouteInfoText>
                    {sheduleItem.emptyPlaces}
                </RouteInfoText>
                <Icon name='car-seat' size={20} color={sheduleItem.emptyPlaces > 10 ? 'green' : sheduleItem.emptyPlaces > 0 ? '#FFB534' : 'red'}/>
            </RouteInfoItem>
            <RouteInfoItem>
                <RouteInfoTextContainer>
                    <RouteInfoText>
                        {sheduleItem.busOwner}
                    </RouteInfoText>
                    <RouteInfoText>
                        {sheduleItem.busInfo}
                    </RouteInfoText>
